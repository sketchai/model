"""
An adaptation of the original Msg Passing + Transformer Architecture, presented in the paper.
This is provided as a baseline
Differences:
    - Constraint parameters are not used
    - Message passing is different for "node to subnode" and "subnode to node" but the layers are essentially the same.
        Instead of an edge embedding, a specific layer is learned for all subnodes. 
"""
import numpy as np
import torch
...
from sketch_gnn.models.msg_passing import SimpleConcatLinearBlock, SubnodeLinearBlock
...
class GaT(pl.LightningModule):
    ...
    def __init__(self, d_model: Dict = {}, d_prep: Dict = {}):
        ...
        for _ in range(self.n_layers):
            edge_msg_passing.append(SimpleConcatLinearBlock(emb_dim=emb_dim))
            subnode_msg_passing.append(SubnodeLinearBlock(emb_dim=emb_dim))
    ...
    def forward(self, data) -> Dict:
        ...
        if self.positional_encoding is not None:
            x = x + self.positional_encoding(data.positions)

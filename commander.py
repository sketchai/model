...
@ex.command
def training(cpu, arch, train, train_data, test_data, path_dataset):
    ...
    with open(os.path.join(path_dataset,train_data['name_param']), 'rb') as f:
    ...
    model = GravTransformer(node_feature_mapping_dim, edge_feature_mapping_dim, 
    arch['embedding_dim'], arch['n_head'], arch['num_layers'], arch['positional_encoding'], lMax)
    model.to(device)

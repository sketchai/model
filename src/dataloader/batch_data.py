...
class GraBatch:
    ...
    def __init__(self, data : Dict[str, torch.Tensor]):
        ...
        self.sparse_node_features_index =  data.get('sparse_node_features_index')
        self.sparse_node_features_value =  data.get('sparse_node_features_value')
        self.incidences = data.get('incidences')
        self.edge_features = data.get('edge_features')
        self.sparse_edge_features_index = data.get('sparse_edge_features_index')
        self.sparse_edge_features_value = data.get('sparse_edge_features_value')
    ...
    def __repr__(self):
        ...
        rep = rep + f'sparse_node_features_index: {self.sparse_node_features_index} \n'
        rep = rep + f'sparse_node_features_value: {self.sparse_node_features_value} \n'
        rep = rep + f'incidences: {self.incidences} \n'
        rep = rep + f'edge_features: {self.edge_features} \n'
        rep = rep + f'sparse_edge_features_index: {self.sparse_edge_features_index} \n'
        rep = rep + f'sparse_edge_features_value: {self.sparse_edge_features_value} \n'
    ...
    def pin_memory(self):
        ...
        for k in self.sparse_node_features.keys():
            self.sparse_node_features_index[k].pin_memory()
            self.sparse_node_features_value[k].pin_memory()
        self.incidences.pin_memory()
        self.edge_features.pin_memory()
        for k in self.sparse_edge_features.keys():
            self.sparse_edge_features_index[k].pin_memory()
            self.sparse_edge_features_value[k].pin_memory()
    ...
    @torch.jit.ignore(drop=True)
    def load_cuda_async(self, device: Optional[torch.Tensor]):

        if device.type != 'gpu':
            return None 

        self.node_features = self.node_features.to(device=device, non_blocking=False)
        for k in self.sparse_node_features_index.keys():
            self.sparse_node_features_index[k]= self.sparse_node_features_index[k].to(device=device, non_blocking=False)
            self.sparse_node_features_value[k] = self.sparse_node_features_value[k].to(device=device, non_blocking=False)
        ...
        for k in self.sparse_edge_features_index.keys():
            self.sparse_edge_features_index[k] = self.sparse_edge_features_index[k].to(device=device, non_blocking=False)
            self.sparse_edge_features_value[k] = self.sparse_edge_features_value[k].to(device=device, non_blocking=False)

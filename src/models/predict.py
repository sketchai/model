...
class PredictSketch(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        with torch.no_grad():
            ...
            l_batch = batch.get('l_batch')
            self.log('metric_to_track',loss, batch_size = l_batch )
            self.log('val_loss', loss, batch_size = l_batch)
            self.log('val_perf_edge.n_edges_pos_predicted_pos', perfs[0][0],batch_size = l_batch)
            self.log('val_perf_edge.n_edges_predicted_pos', perfs[0][1],batch_size = l_batch)
            self.log('val_nb_edges_pos',perfs[0][2], batch_size = l_batch)
